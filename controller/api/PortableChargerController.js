...
export const userCancelPCBooking = asyncHandler(async (req, resp) => {
    ...
    const checkOrder = await queryDB(`
        SELECT  
            pcb.vehicle_data,pcb.rsa_id, pcb.address, pcb.slot_time, pcb.user_name, DATE_FORMAT(pcb.slot_date, '%Y-%m-%d') AS slot_date, concat( pcb.country_code, "-", pcb.contact_no) as contact_no, riders.rider_email , riders.rider_name, riders.fcm_token, rsa.fcm_token as rsa_fcm_token
        FROM  
    ...
    try {
        ...
        const htmlUser = `<html>
            <body>
                ...
                Booking ID: ${booking_id}<br>
               Rescheduled Date & Time: ${moment(fSlotDate, 'YYYY MM DD').format('D MMM, YYYY,')} ${moment(slot_time, 'HH:mm').format('h:mm A')}<br>
                <p>Thank you for choosing PlusX Electric. </p>                  
                <p> If you have any questions or need further assistance, feel free to contact us. </p><br/>
                <p>Best regards,<br/>PlusX Electric Team</p>
            </body>
        </html>`;
        ...
        const htmlAdmin = `<html>
            <body>
                <h4>Dear Admin,</h4>
                <pThis is to inform you that a user has rescheduled their Portable EV Charging Service booking. Please find the updated booking details below:</p>
             User Name : ${checkOrder.user_name}<br>
              User Contact: ${checkOrder.country_code}-${checkOrder.contact_no}<br>
               ...
               New Scheduled Date & Time: ${moment(fSlotDate, 'YYYY MM DD').format('D MMM, YYYY,')} ${moment(slot_time, 'HH:mm').format('h:mm A')}<br>       
       ...
       /* const href = 'portable_charger_booking/' + booking_id;
                ...
                if(rsa_id!='')   pushNotification(rsa_fcm_token, heading, desc, 'RDRFCM', href);*/
